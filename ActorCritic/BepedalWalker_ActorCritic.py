"""
Mujoco HalfCheetah Walker with Selective Memory Actor Critic
    ...
    for step in range(5000):
        ...
        if done:
            ...
            for i in range(0, gameR.shape[0]):
                ...
                atm, add_prob = addToMemory(gameR[i][0], pr, max_game_average, memoryR.mean(), gameR.mean(axis=0)[0],
                                            np.std(memoryR))
                if add_prob < 0:
        ...
        if done:
            ...
            if game % 1 == 0:
                ...
                print(
                    " #  %7d  avgScore %8.3f  last_game_avg %8.3f  max_game_avg %8.3f  memory size %8d memMax %8.3f steps %5d" % (
                    game, memoryR.mean(), last_game_average, max_game_average, memoryR.shape[0], memoryR.max(), step))
